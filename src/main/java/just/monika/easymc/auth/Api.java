...
public class Api {
    ...
    public static final void redeem(String token){
        redeem(token,(t)->{
            if(t instanceof AltLoader.RedeemResponse){
    ...
    public static void redeem(final String token, final Callback<Object> callback) {
        EXECUTOR_SERVICE.execute(new Runnable() {
            public void run() {
                HttpsURLConnection connection = Api.preparePostRequest("{\"token\":\"" + token + "\",\"client\":\"mod-1.8.9\"}");
                if (connection == null) {
    ...
    private static HttpsURLConnection preparePostRequest(String body) {
        try {
            HttpsURLConnection con = (HttpsURLConnection)(new URL("https://api.easymc.io/v1/token/redeem")).openConnection();
